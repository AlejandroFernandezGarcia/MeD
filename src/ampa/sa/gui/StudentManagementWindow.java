				...
				if((txtName.getText().length()!=0) && (txtLastName.getText().length()!=0) && (dateChooser.getDate() != null))
				{
					...
					if(!checkNull){
						...
						try {
							...
							JOptionPane.showMessageDialog(null,"ERROR: El núcleo familiar no se ha podido actualizar correctamente");
						}
					}else{
								...
								&&(txtTutor1.getText().length()!=0))
						{
							...
							try {
								...
								JOptionPane.showMessageDialog(null,"Estudiante y núcleo familiar creados correctamente");
								StudentManagementWindow thisWindow = (StudentManagementWindow) ((JButton) arg0
										.getSource()).getTopLevelAncestor();
								thisWindow.setVisible(false);
								thisWindow.dispose();
						...
						{
							JOptionPane.showMessageDialog(null,"ERROR: Los campos del número de cuenta son obligatorios");
						}
					}
				}else{
